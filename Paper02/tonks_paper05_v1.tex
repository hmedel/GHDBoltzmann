...
\begin{abstract}
The one-dimensional hard-rod gas is integrable: the
single-species kinetic operator admits an extensive family
of conserved charges (rapidities equal to bare velocities),
and the linearised Chapman--Enskog formulation reproduces
the known anomalous heat conduction in that limit. We
construct a discrete-velocity numerical solver of the
two-species generalised hydrodynamic (GHD) Boltzmann
equation of Ref.~\cite{PaperIV} for the binary mass-disordered
hard-rod gas --- the simplest deformation that breaks the
integrability of the single-species limit --- and extract
its leading kinetic transport coefficients from a 3-point
Lagrange Bobylev--Palczewski--Schneider discretisation that
preserves the four collision invariants of the linearised
operator to machine precision. The thermal conductivity
$\lambda_T(m_{\max})$ exhibits a power-law divergence
$\lambda_T\sim (m_{\max}-1)^{-\alpha}$ with
$\alpha\approx 1.6$ as the integrable single-species limit
is approached, providing the first first-principles
quantification of the integrability-to-diffusion crossover
in this system; the binary interdiffusion coefficient
$D_{LH}(m_{\max})\approx 4$ remains finite throughout. The
solver itself reproduces the analytical instantaneous group
velocity of Ref.~\cite{PaperIV} to $0.1\%$ across a 20-fold
range of mass disparity, satisfies the H-theorem, and
recovers both the linear-regime $1/n$ odd-mode density-%
amplitude ratio and the nonlinear-regime even-mode
saturation of Ref.~\cite{PaperIII}. At the experimental
wavelength $k_1$ of Refs.~\cite{PaperII,PaperIII} the
Knudsen number $\mathrm{Kn}\approx 3.5$ places the system
\emph{outside} the small-$k$ hydrodynamic regime of the
Chapman--Enskog expansion; direct diagonalisation of the
linearised operator at $k_1$ then shows that the sound-mode
damping is set by the collision rate $\tau_{\rm break}^{-1}$
rather than by the transport coefficients, in quantitative
agreement with both the DVM-measured
$\gamma^{\rm DVM}\approx 0.04$ and the MD-empirical
...
Section~\ref{sec:nu} extracts the two relevant transport
coefficients $D_{LH}(m_{\max})$ and $\lambda_T(m_{\max})$
from the first-order Chapman--Enskog correction, identifies
the integrability-to-diffusion crossover signalled by the
growth of $\lambda_T$ as $m_{\max}\to 1$, and resolves the
sound-mode damping puzzle by computing the full dispersion
relation $\omega(k)$ of the linearised GHD--Boltzmann
operator: at the experimental $k_1\approx 0.50$, the
Knudsen number is $\mathrm{Kn}\approx 3.5$ and the damping
is collision-rate-limited.
...
%======================================================================
\section{Transport coefficients and the kinetic regime}\label{sec:nu}
%======================================================================
...
Sec.~\ref{sec:integrability}). The binary mass disorder
...
verify this numerically to machine precision, $|L_{nT}/L_{Tn}-1|<10^{-14}$.
The cross-coefficient values are not central to our
analysis (they do not affect the leading sound damping in
the kinetic regime) and are reported in the supplementary
material. Positivity $D_{LH}>0$ and $\lambda_T>0$ at all
$m_{\max}$ tested follows from the negative-semidefiniteness
of $\mathsf{L}$ in the inner product
Eq.~\eqref{eq:innerprod}, which we verify by the eigenvalue
spectrum of the symmetrised operator
$\mathsf{D}^{1/2}\mathsf{L}\mathsf{D}^{-1/2}$ being
non-positive to numerical precision.

The discrete H-theorem (Sec.~\ref{sec:dvm} item~(c))
provides a complementary thermodynamic-consistency check:
the kinetic entropy $S(t)$ is non-decreasing along the full
DVM evolution to within
$|dS/dt|/S\lesssim 10^{-6}$ per collision step, with the
small residual reflecting the off-grid truncation of the
collision kernel rather than a sign violation. The
linearised entropy production rate near equilibrium is
$\dot{S}^{(2)}=\langle h,-\mathsf{L} h\rangle\geq 0$,
manifestly non-negative because $-\mathsf{L}$ is positive
semi-definite. The transport coefficients $D_{LH}$ and
$\lambda_T$ extracted via Eq.~\eqref{eq:Dnum},
\eqref{eq:qT} are then the standard linear-response
coefficients of this entropy-production-rate functional.

\subsection{Summary}
\label{sec:nu-summary}

The two transport coefficients $D_{LH}(m_{\max})$ and
$\lambda_T(m_{\max})$ are the leading dissipative
hydrodynamic coefficients of the linearised binary
GHD--Boltzmann equation, well-defined in the
small-Knudsen-number limit. They exhibit complementary
physical signatures: $D_{LH}$ is slowly varying in
$m_{\max}$ at the level of the Maxwell--Stefan-like
inter-species mass transport, while $\lambda_T$ encodes the
integrability-to-diffusion crossover with a power-law
divergence $\lambda_T\sim (m_{\max}-1)^{-\alpha}$,
$\alpha\approx 1.6\pm 0.1$ extracted by linear regression
over $m_{\max}\in[1.1,5]$ (three independent data points
$\lambda_T=1.0\times 10^5,5.4\times 10^3,3.8\times 10^2$;
the uncertainty $\pm 0.1$ on $\alpha$ is the residual to a
strict power-law fit). The full dispersion-relation
analysis of Secs.~\ref{sec:dispersion}--%
\ref{sec:kinetic-regime} then shows that the experimental
sound-mode damping rate observed at $k_1=2\pi/L$ in
Refs.~\cite{PaperII,PaperIII} is not given by the
Navier--Stokes formula $\gamma\propto k^2$ in terms of
these transport coefficients but by the kinetic-regime
collision rate $\tau_{\rm break}^{-1}$, in quantitative
agreement with DVM and MD measurements.
...
Sec.~\ref{sec:integrability}: the thermal conductivity
