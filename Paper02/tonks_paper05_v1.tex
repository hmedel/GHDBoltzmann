...
mildly toward $m_{\max}\to 1$. This contradicts a naive
expectation that the interdiffusion would vanish in both the
equal-mass and infinite-mass-ratio limits: the discrete
species-label exchange under the GHD--Boltzmann collision
operator does not commute with the velocity swap induced by
the elastic-collision map, so interdiffusion persists as a
non-trivial relaxation channel even at $m_{\max}=1$ (where
the masses are equal but the labels remain distinct kinetic
degrees of freedom). In the heavy-disparity limit the
kinematic prefactor of $D_{LH}$ does not vanish either; the
heavy species behaves as a near-static scatterer that
remains efficient at exchanging the LIGHT-species composition
gradient with the local Maxwell--Boltzmann velocity
distribution.

\subsection{From $D_{LH}$ to the sound-wave damping rate}
