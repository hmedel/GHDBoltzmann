...
physical signatures: $D_{LH}$ is essentially flat in
$m_{\max}$ at the level of the
Maxwell--Stefan-like inter-species mass transport,
while $\lambda_T$ encodes the integrability-to-diffusion
crossover. The full dispersion-relation analysis of
Secs.~\ref{sec:dispersion}--\ref{sec:kinetic-regime}
then shows that the experimental sound-mode damping rate
observed at $k_1=2\pi/L$ in Refs.~\cite{PaperII,PaperIII}
is not given by the Navier--Stokes formula
$\gamma\propto k^2$ in terms of these transport
coefficients but by the kinetic-regime collision rate
$\tau_{\rm break}^{-1}$, in quantitative agreement with
DVM and MD measurements.
