...
\begin{equation}
\lambda_T(m_{\max}) \sim
A\,(m_{\max}-1)^{-\alpha}, \quad \alpha \approx 1.6
...
slope $-1.59$ over this range; the $m_{\max}=1.001$ point
is below numerical resolution of the binary structure and is
discarded). The exponent $\alpha\approx 1.6$ is the kinetic
...
finite, scaling with $(m_{\max}-1)^{-1.6}$ in the window
...
$\alpha\approx 1.6$ from the perturbative structure of the
...
The time evolution of $|\hat\rho_n(t)|$ for $n=1,\dots,7$
over twelve sound periods at the linear amplitude $U_0=0.05$
is recorded in the simulation data \texttt{step\_sim\_step\_U005.csv}. The peak amplitudes
...
$(m_{\max}-1)^{-\alpha}$ with $\alpha\approx 1.6$
