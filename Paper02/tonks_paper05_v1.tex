...
transport coefficients, the integrability-to-diffusion
crossover, and the kinetic-regime sound mode}
...
integral. We construct a discrete-velocity numerical solver
of the GHD--Boltzmann equation with Strang operator splitting
and a three-point Lagrange interpolation collision kernel
that preserves the four collision invariants to machine
precision. The solver recovers the analytical sound speed of
Ref.~\cite{PaperIV} to $0.1\%$ across a 20-fold range of mass
disparity, satisfies the H-theorem, and reproduces the
linear-regime $1/n$ odd-mode density-amplitude ratio and the
nonlinear-regime even-mode saturation of Ref.~\cite{PaperIII}.
From the first-order Chapman--Enskog correction to local
Maxwell--Boltzmann equilibrium we extract two transport
coefficients: the binary interdiffusion coefficient
$D_{LH}(m_{\max})\approx 4.0$ and the thermal conductivity
$\lambda_T(m_{\max})$. The latter exhibits a dramatic
crossover from anomalous (divergent) to normal (diffusive)
behaviour as $m_{\max}$ increases from the integrable
single-species limit, providing the first first-principles
quantification of the integrability-to-diffusion transition
in this system. Direct diagonalisation of the linearised
operator shows that at the experimental wavelength $k_1$ of
Refs.~\cite{PaperII,PaperIII} the system is in the
\emph{kinetic} regime ($\mathrm{Kn}\approx 3.5$), where the
sound-mode lifetime is set by the inter-species collision
rate $\tau_{\rm break}^{-1}$ rather than by a hydrodynamic
transport coefficient. The predicted apparent damping
$|\mathrm{Im}\,\omega|\approx 0.02$ matches the DVM-measured
$\gamma^{\rm DVM}\approx 0.04$ within a factor of two and is
...
toward smaller $c_s$. At $m_{\max}=5$ the ratio
...
$m_{\max}=5,10,100$), confirming that the DVM solver
...
\begin{equation}
b_\alpha^{(T)}(v) = (v/T)\bigl(m_\alpha v^2/(2T) - 3/2\bigr),
\label{eq:bT}
\end{equation}
...
$\psi_3=m_\alpha v$. The corresponding heat flux,
\begin{equation}
q = \!\int\!dv\,v
...
\label{eq:qT}
\end{equation}
defines the thermal conductivity $\lambda_T = -q$ at unit
gradient $\partial_s T=1$.
...
linearised operator, with the thermal source Eq.~\eqref{eq:bT}
and the heat-flux moment Eq.~\eqref{eq:qT} defined in
Sec.~\ref{sec:nu}. The numerical extraction at the same
parameters ($\eta=0.1$, $N_v=160$, $V_{\max}=8$, equimolar)
gives
\begin{equation}
\begin{array}{c|cccccc}
...
two non-trivial transport coefficients
$D_{LH}(m_{\max})\approx 4.0$ (essentially flat in
$m_{\max}$) and $\lambda_T(m_{\max})$ (decreasing from
$\approx 1845$ at $m_{\max}=2$ to $\approx 212$ at
...
A by-product of this analysis is the
\emph{integrability-to-diffusion crossover} reported in
Sec.~\ref{sec:integrability}: the growth of
$\lambda_T(m_{\max})$ as $m_{\max}\to 1$ is the kinetic
signature of the anomalous heat conduction of the
single-species 1D hard-rod gas, restored to normal
Fourier-law diffusion by the binary mass disorder. To our
knowledge this is the first first-principles quantification
of this crossover in the hard-rod system.
