...
noise-averaged limit --- and identify a parametric delay
threshold $\tau_r^c\approx 2.8$ (in the natural time units
of Ref.~\cite{PaperIII}, where $\tau_{\rm break}\approx 9$)
for the onset of the Burgers cascade, with the inelastic
channel $\epsilon>0$ \emph{suppressing} rather than enhancing
shock formation. Direct numerical tests with two
microscopic delay implementations --- a post-collisional
contact-freeze and an anticipatory optimal-velocity-model
force --- show that this continuum threshold is \emph{not
realised}: the contact-freeze delay \emph{suppresses}
even-mode generation below the elastic baseline
($R_2$ drops from $0.16$ to $0.02$--$0.05$), and the
anticipatory delay shows no monotonic $R_2(\tau_r)$
crossover at any coupling strength. The $O(10^2)$ elastic
collisions occurring during the delay interval decorrelate
the delayed gap information, and the integrable hard-rod
substrate shields the system against shock formation.
The tension between the continuum prediction and the
microscopic null result identifies integrability as the
obstacle to delay-induced shock onset.
\end{abstract}
...
caricature, while the car-following level is captured by
the Optimal Velocity Model
\cite{Bando1995,Komatsu1995,Komatsu2003} and the Intelligent
...
\emph{(b) Reaction-time delay $\tau_r$.} The delay can be
implemented in two ways, both tested in
Sec.~\ref{sec:numerical}: a \emph{post-collisional}
contact-freeze rule, in which both rods adopt
$v_{\rm min}=\min(v_i,v_j)$ at the contact event $t_c$
and remain in contact until the elastic exchange is applied
at $t_c+\tau_r$; and an \emph{anticipatory} OVM-type force
$dv_i/dt=\alpha[V_{\rm opt}(g_i(t-\tau_r))-v_i]$, in which
each rod adjusts its velocity based on the gap sensed
$\tau_r$ time units in the past, with elastic hard-core collisions as
a safety-net constraint. Both reduce to the standard
instantaneous-swap dynamics for $\tau_r\to 0$ and preserve
the cyclic-order invariant. The symbolic
$F_i^{\rm delay}$ in Eq.~\eqref{eq:micro2} stands for
either implementation and breaks instantaneous
reversibility; the explicit protocols are in
...
\emph{(b)} reaction-time delay $\tau_r>0$ is predicted by
the continuum theory to \emph{sustain} coherent flow against
thermalisation (the dissipative response is suspended during
the delay) and thus \emph{lengthen} $\tau_{\rm coh}$,
promoting shocks --- consistent with empirical jam-wave
formation in delayed car-following
models~\cite{Bando1995,Treiber2000,Komatsu1995}. Whether
this mechanism operates at the microscopic level depends on
the scale-separation hypothesis tested in
Sec.~\ref{sec:numerical}.
...
Table~\ref{tab:scans} summarises the numerical campaigns.

\begin{table}[htbp]
\caption{Summary of numerical scans. All use $N=400$,
  $\eta=0.10$, $T_{\rm sim}=135\approx 12\,\tau_{\rm sound}$,
  step IC at $U_0=0.5$.}
\label{tab:scans}
\centering
\begin{tabular}{lcccc}
\hline\hline
Scan & Model & $\alpha$ & Seeds & $\tau_r$ grid \\
\hline
B    & contact-freeze & --- & 200 & 0--4 (7 pts) \\
OVM\,1 & OVM-HR & 0.1--1.0 & 20  & 0--4 (5 pts) \\
OVM\,2 & OVM-HR & 0.05--0.15 & 30  & 0--8 (7 pts) \\
Res. & OVM-HR & 0.10, 0.12 & 100 & 0--11 (12 pts) \\
\hline\hline
\end{tabular}
\end{table}
...
the deterministic GHD--Boltzmann solver of
Ref.~\cite{PaperV} (Table~VI) reports $R_2\approx 0.043$ at
$U_0=0.5$, while the molecular-dynamics baseline at $N=400$
and 200 realisations gives a higher value $R_2\approx 0.16$
due to thermal noise in the ensemble-averaged Fourier
amplitudes. Formation of a Burgers shock breaks this parity
...
the MD baseline to $O(0.5)$ would confirm Scenario~A;
...
\tau_{\rm sound}$ yields $R_2\approx 0.16$ at
$\tau_r=0$ (the integrable baseline) and
$R_2\approx 0.02$--$0.05$ for every $\tau_r>0$
(Fig.~\ref{fig:R2}a)---the delay \emph{suppresses}
even-mode generation below the elastic baseline.
...
Across the full parameter space explored
(Fig.~\ref{fig:R2}b), no monotonic
...
\begin{figure}[htbp]
\includegraphics[width=\columnwidth]{figures/fig_p6_R2_all.pdf}
\caption{Even-mode ratio $R_2\equiv|\hat\rho_2|/|\hat\rho_1|$
  evaluated at the first peak of $|\hat\rho_1|$ versus delay
  $\tau_r$. (a)~Model~B (contact-freeze, 200 realisations):
  $R_2\approx 0.16$ at $\tau_r=0$ (integrable baseline) drops
  to $R_2\approx 0.02$--$0.05$ for all $\tau_r>0$, well below
  the predicted threshold $\tau_r^c\approx 2.8$ (red dash-dot).
  Error bars show $\pm 1\sigma$.
  (b)~OVM-HR model (30 realisations) at four coupling strengths
  $\alpha$: no monotonic crossover is observed at any $\alpha$.
  The $\alpha=0.15$ curve (dashed) already exceeds $R_2=0.5$ at
  $\tau_r=0$, while $\alpha\leq 0.08$ remains near the floor.
  The intermediate value $\alpha=0.12$ shows large but
...
$\tau_{\rm sound}/2\approx 5.7$ (12 values of $\tau_r$ from
...
acoustic period (Fig.~\ref{fig:resonance}). The result is negative:
...
\begin{figure}[htbp]
\includegraphics[width=\columnwidth]{figures/fig_p6_resonance.pdf}
\caption{Resonance test: $R_2$ versus $\tau_r/\tau_{\rm sound}$
  for $\alpha=0.10$ and $\alpha=0.12$ (100 realisations each).
  Error bars show $\pm 1\sigma$ from the ensemble variance.
  The red dash-dot line marks $\tau_r=\tau_{\rm sound}/2$,
  the expected resonance location. No peak is observed at
  $\tau_r/\tau_{\rm sound}=1/2$ for either coupling. The
  $\alpha=0.12$ data shows large non-monotonic excursions
  (e.g., a deep trough at $\tau_r/\tau_{\rm sound}\approx 0.58$
  between elevated values at $0.53$ and $0.71$), inconsistent
  with a smooth resonance structure.}
\label{fig:resonance}
\end{figure}
...
\paragraph*{Decorrelation timescale and diagnostic choice.}
The gap--gap autocorrelation
$C_g(\tau)=\langle g_i(t)\,g_i(t+\tau)\rangle$ decays on the
collision timescale $\tau_{\rm coll}\approx 0.03$: after
$n\sim\tau_r/\tau_{\rm coll}\sim 10^2$ independent elastic
scatterings, the delayed gap $g_i(t-\tau_r)$ retains no
memory of the current configuration. The effective
decorrelation rate $\sigma_{\rm dec}\sim\tau_{\rm coll}^{-1}
\approx 30$ thus exceeds the Burgers steepening rate
$t_{\rm shock}^{*\,-1}\approx 0.25$ by two orders of
magnitude, confirming that collisional scrambling pre-empts
nonlinear steepening. A direct measurement of $C_g(\tau)$
from the simulation trajectories would provide a quantitative
test of this estimate and is left to future work.

Additionally, the ratio diagnostic
$R_2=|\hat\rho_2|/|\hat\rho_1|$ amplifies statistical
noise when $|\hat\rho_1|$ passes through a minimum: the
$|\hat\rho_1|_{\rm pk}$ denominator varies by a factor of
two across the $\tau_r$ grid in the resonance scan,
inflating $R_2$ fluctuations. The absolute even-mode
amplitude $|\hat\rho_2|$ is immune to this ratio
sensitivity; inspection of the raw $|\hat\rho_2|_{\rm pk}$
values (Figs.~\ref{fig:R2}--\ref{fig:resonance}) confirms
that the even-mode amplitude itself remains at or below
the thermal noise floor for all $\tau_r$, reinforcing the
null result independently of the choice of diagnostic.
...
derivation~\cite{Komatsu1995}. For example, at $c_s\approx 1$
and $\tau_r=2.8$ (the predicted threshold), this gives
$\nu_\tau\approx 2.8$, an order of magnitude larger than
$\nu_0\approx 0.4$. The perturbative expansion
$O(\tau_r^2/\tau_{\rm break})$ in Eq.~\eqref{eq:tcoh_tau}
is therefore only marginally controlled at $\tau_r^c$, as
noted in Sec.~\ref{sec:onset}.
