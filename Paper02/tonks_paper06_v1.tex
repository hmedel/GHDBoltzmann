...
event-driven dynamics, and companion paper~\cite{PaperIV} traced
this to the absence of dissipative or non-Markovian channels in
the GHD--Boltzmann equation. Vehicular traffic, by contrast,
exhibits robust shock-like jam waves. We construct a minimal
three-level hierarchy --- microscopic stochastic Langevin
dynamics with reaction-time delay and inelastic restitution,
mesoscopic stochastic Burgers equation for the coarse-grained
velocity field, and macroscopic deterministic Burgers equation
in the noise-averaged limit --- and identify the parametric
threshold at which the coherent shock signature emerges. The
mesoscopic stochastic Burgers equation provides the natural
bridge between integrable hard-rod phenomenology
(Refs.~\cite{PaperI,PaperII,PaperIII,PaperIV}) and the
shock-dominated regime relevant for traffic flow.
\end{abstract}
...
Vehicular traffic, by contrast, exhibits robust shock-like
jam waves~\cite{Sugiyama2008}, well-described at the continuum
level by Lighthill--Whitham--Richards
theory~\cite{Lighthill1955} and at the car-following level by
...
continuum;
\emph{(iii)} a parametric threshold for shock onset that
provides a quantitative target for car-following experimental
realisations~\cite{Sugiyama2008}.
...
\emph{(b) Reaction-time delay $\tau_r$.} The delay force
$F_i^{\rm delay}$ encodes the driver-like response to a
neighbour state $\tau_r$ in the past,
$F_i^{\rm delay}=F[v_i(t),v_{i+1}(t-\tau_r),s_{i+1}(t-\tau_r)
-s_i(t)]$. This breaks instantaneous reversibility and is
the canonical traffic-model ingredient
\cite{Bando1995,Treiber2000}.
...
D_{\rm eff}(\epsilon,D)
  &= D + D_\epsilon\,\epsilon
     ...
     \label{eq:D_eff}
\end{align}
with explicit prefactors derived in Appendix~B. The elastic limit $\epsilon=\tau_r=0$ gives $\nu_0$ equal
...
\subsection{Density-of-states evolution and KPZ universality}

Equation~\eqref{eq:stoch_burgers} is the velocity-field form
of the Kardar--Parisi--Zhang (KPZ) equation~\cite{KPZ1986}
$\partial_t h = \nu\,\partial_x^2 h + (\partial_x h)^2/2
+ \sqrt{D}\,\eta$ with $u = -\partial_x h$. The KPZ universality
class governs the long-wavelength fluctuations of $u$ in the
mesoscopic regime, and predicts characteristic dynamic
exponent $z=3/2$ and roughness exponent $\alpha=1/2$ for the
height field~\cite{Hairer2013,QuastelSpohn2015}. The
density-of-states function $f(x,v,t)$ of
Eq.~\eqref{eq:fp} therefore evolves at the mesoscopic scale
according to a stochastic dynamics in the KPZ universality
class, whose roughening properties at $u\to$ const
backgrounds we use as the diagnostic of shock onset
(Sec.~\ref{sec:onset}).
...
\label{eq:tcoh_tau}
\end{equation}
to leading order in $\tau_r/\tau_{\rm break}$. The
$O(\tau_r^2)$ correction arises from the renormalisation of
the collision frequency at finite delay and is neglected in
the linear estimate below. Shock onset corresponds to
$\tau_{\rm coh}(\tau_r)\geq t^*_{\rm shock}$; setting
equality and using
$t^*_{\rm shock}=1/(k_1 u_0)\approx 1/(0.5\cdot 0.5)=4$
and the measured
$\tau_{\rm coh}(0)\approx 1.2$ of Ref.~\cite{PaperIII}
gives the leading-order threshold
\begin{equation}
\boxed{\;\tau_r^c \;\approx\; t^*_{\rm shock}
- \tau_{\rm coh}(0) \;\approx\; 4 - 1.2 \;=\; 2.8,\;}
\label{eq:tauc}
\end{equation}
the minimum reaction-time delay required for the Burgers
cascade to appear at the parameters of
Ref.~\cite{PaperIII}. The estimate is leading-order in
$\tau_r/\tau_{\rm break}\approx 0.3$ at $\tau_r^c$, with the
sub-leading correction of order $30\%$; a tighter
numerical determination of $\tau_r^c$ is the principal
target of the protocol of Sec.~\ref{sec:numerical}.

\emph{Combined channel.} With both $\epsilon$ and $\tau_r$
active, Eqs.~\eqref{eq:tcoh_eps}--\eqref{eq:tcoh_tau} compete,
and the threshold curve in the $(\epsilon,\tau_r)$ plane
slopes upward: increasing $\epsilon$ requires larger $\tau_r$
to compensate for the inelastic suppression.

\subsection{Qualitative phase diagram}

The expected $(\epsilon,\tau_r)$ phase diagram has three
regions:
\emph{(I) No-shock subsonic} ($\tau_r<\tau_r^c$, any
$\epsilon$): phase mixing pre-empts shock formation, the
$|\hat\rho_n|/|\hat\rho_1|=1/n$ ratio of Ref.~\cite{PaperIV}
persists, the system is in the GHD-Boltzmann regime.
\emph{(II) Marginal} ($\tau_r\approx\tau_r^c$, $\epsilon\ll
1$): the shock cascade onsets, the ratio
$|\hat\rho_n|/|\hat\rho_1|$ transitions from $1/n$ toward
constant in $n$.
\emph{(III) Burgers-dominated} ($\tau_r>\tau_r^c$, any
$\epsilon$): coherent shock develops with KPZ-class
fluctuations of the noise term in
Eq.~\eqref{eq:stoch_burgers}; the
$|\hat\rho_n|/|\hat\rho_1|\sim 1$ Burgers cascade emerges.

The transition at $\tau_r^c\approx 2.8$ is the principal
...
The delay channel is implemented by replacing the
instantaneous collision rule of Ref.~\cite{PaperIV} by a
deferred rule in which the post-collision velocity of rod
$i$ at contact event $t_c$ takes effect at time
$t_c+\tau_r$, with $i$ continuing on its pre-collision
trajectory in the interval $(t_c,t_c+\tau_r)$. This is the
hard-rod analogue of the OVM reaction-time
delay~\cite{Bando1995} compatible with event-driven
integration. The inelastic channel is fixed at $\epsilon=0$
to isolate the delay-only shock onset.
...
The diagnostic signature of shock onset is the transition
of $R_n$ from the $1/n$ structure (linear-regime cascade,
Ref.~\cite{PaperIV}, no Burgers shock) to a constant-$n$
plateau $R_n\approx R_*$ characteristic of the
inviscid-Burgers fixed point of a step IC. Equivalently,
the cumulant
$\kappa\equiv R_2 R_4 / R_1 R_3$ vanishes in the
$1/n$ regime and approaches unity in the Burgers regime
(triangle inequality for the inviscid-Burgers Fourier
amplitudes).

\paragraph*{Expected result.}
For $\tau_r<\tau_r^c$: phase mixing pre-empts shock
formation, the linear-regime cascade $R_n\sim 1/n$ persists
(consistent with Ref.~\cite{PaperIII}).
For $\tau_r\approx\tau_r^c$: marginal onset, $R_n$ flattens
with $n$ but the cascade is incomplete.
For $\tau_r>\tau_r^c$: full Burgers cascade,
$R_n\to R_*\approx 0.5$--$0.7$, indistinguishable from the
inviscid-Burgers Fourier signature within the
discretisation noise.

The quantitative match between the threshold extracted
from the $R_n$ data and the analytical prediction
...
Refs.~\cite{PaperI,PaperII,PaperIII}) rather than from a
phenomenological Boltzmann-type ansatz on the desired-velocity
distribution.
...
direct Langevin integration of
Eqs.~\eqref{eq:micro1}--\eqref{eq:micro2}) and analytically
via the well-developed KPZ machinery
\cite{Hairer2013,QuastelSpohn2015}.
The principal open computational task is the direct
numerical determination of the threshold $\epsilon_c(\tau_r,
D)$ from event-driven simulations of the microscopic dynamics,
and its comparison against the analytical prediction
Eq.~\eqref{eq:tauc}.

For vehicular-traffic applications, the result implies that
shock-like jam waves emerge only when at least one of the
three channels --- inelasticity, reaction-time delay, or
external driver noise --- is sufficiently strong; an elastic,
instantaneous, deterministic car-following model cannot
reproduce the empirical phenomenology of jam-wave formation
documented in Ref.~\cite{Sugiyama2008}.
...
\mathcal{C}_{\rm coll}^{(\epsilon)}[f]
= \sigma\!\int dv'\,|v-v'|\\
\times\left[\frac{1}{r^2}\,f(v_*)\,f(v'_*)
            - f(v)\,f(v')\right],
\label{eq:Cinel}
\end{multline}
...
the post-collision velocities $v_i', v_j'$ are generically
off-grid, and the bilinear $2$-point scheme that suffices
in the elastic case breaks the discrete momentum and energy
balance to $O(\Delta v^2)$ \emph{plus} an $O(\epsilon)$ error
from the truncated Jacobian. The $3$-point Lagrange scheme
preserves momentum and energy balance to machine precision
for any $r\in[0,1]$.
...
A fluctuation--dissipation argument relates the inelastic
dissipation rate to an enhanced noise: the bath maintaining
the kinetic temperature must inject an energy flux equal to
the inelastic loss, giving
\begin{equation}
D_\epsilon = \nu_\epsilon\,k_BT/\bar m,
\end{equation}
so that $D_{\rm eff} = D + D_\epsilon$ grows linearly with
$\epsilon$. In the noise-driven limit $D\gg D_\epsilon$, the
stochastic Burgers equation Eq.~\eqref{eq:stoch_burgers} is
in the standard KPZ regime~\cite{KPZ1986}.
