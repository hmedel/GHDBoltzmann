...
collision, the delay is implemented at the level of the
collision rule rather than as a continuous force: the
post-collision velocity update of rod $i$ at contact event
$t_c$ takes effect only at time $t_c+\tau_r$, with $i$
continuing on its pre-collision trajectory in the interval
$(t_c,t_c+\tau_r)$ (the explicit numerical protocol is in
Sec.~\ref{sec:numerical}). This is the hard-rod analogue
of the reaction-time delay of the Optimal Velocity Model
and the Intelligent Driver Model
\cite{Bando1995,Treiber2000}; the symbolic
$F_i^{\rm delay}$ in Eq.~\eqref{eq:micro2} stands for this
deferred collision rule and breaks instantaneous
reversibility.
...
regions (Fig.~\ref{fig:phase}):
\emph{(I) No-shock} ($\tau_r<\tau_r^c$, any
$\epsilon$): phase mixing pre-empts shock formation, the
odd-mode $|\hat\rho_n|/|\hat\rho_1|=1/n$ cascade of
Ref.~\cite{PaperIV} persists, even modes remain at the
noise floor $R^{\rm e/o}\lesssim 0.15$ of Ref.~\cite{PaperV}.
\emph{(II) Marginal} ($\tau_r\approx\tau_r^c$):
the quadratic mode-mixing channel opens and
$R^{\rm e/o}$ rises monotonically with $\tau_r$.
\emph{(III) Burgers-dominated} ($\tau_r>\tau_r^c$):
a coherent steep front develops with KPZ-class
fluctuations of the noise term in
Eq.~\eqref{eq:stoch_burgers}; the even-to-odd ratio
saturates at $R^{\rm e/o}\sim O(1)$.

The threshold curve $\tau_r^c(\epsilon)$ slopes upward in
the $(\epsilon,\tau_r)$ plane because $\epsilon>0$
shortens $\tau_{\rm coh}$ and therefore demands a larger
$\tau_r$ to compensate
(Eqs.~\eqref{eq:tcoh_eps}--\eqref{eq:tcoh_tau}). The
transition at $\tau_r^c(0)\approx 2.8$ is the principal
...
$m_{\max}=5$, step IC). The dashed curve is the
qualitative threshold $\tau_r^c(\epsilon)$ from
Eqs.~\eqref{eq:tcoh_eps}--\eqref{eq:tcoh_tau}; the
elastic-limit value $\tau_r^c(0)\approx 2.8$ is from
Eq.~\eqref{eq:tauc}. Region~I is the no-shock GHD--Boltzmann
...
therefore the \emph{even-to-odd ratio}
$R^{\rm e/o}\equiv|\hat\rho_2|/|\hat\rho_3|$ at the first
maximum of $|\hat\rho_1|$: in the GHD--Boltzmann
no-shock regime $R^{\rm e/o}\ll 1$ at $U_0=0.5$
(Ref.~\cite{PaperV} measures $R^{\rm e/o}\approx 0.14$ at
$U_0=0.5$, i.e.\ even modes still suppressed by an order
of magnitude), while in the Burgers-shock regime the
nonlinear mixing pushes $R^{\rm e/o}\to O(1)$.

\paragraph*{Expected result.}
For $\tau_r<\tau_r^c$: phase mixing pre-empts shock
formation, the even-to-odd ratio remains at the
no-shock floor $R^{\rm e/o}\lesssim 0.15$
(Ref.~\cite{PaperV}, Ref.~\cite{PaperIII}).
For $\tau_r\approx\tau_r^c$: $R^{\rm e/o}$ rises as the
quadratic mode-mixing channel opens but the cascade is
incomplete.
For $\tau_r>\tau_r^c$: $R^{\rm e/o}\to O(1)$, signalling
the developed Burgers cascade.
