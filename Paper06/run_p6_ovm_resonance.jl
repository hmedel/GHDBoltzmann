#!/usr/bin/env julia
"""
P6 OVM-HR resonance scan: fine τ_r grid around τ_sound/2 ≈ 5.5.
Tests parametric resonance hypothesis: R₂ peaks when τ_r ≈ τ_sound/2.

α ∈ {0.10, 0.12} (sweet spot from scan 2)
τ_r ∈ {0, 2, 3, 4, 4.5, 5.0, 5.5, 6.0, 6.5, 7.0, 8, 11} (fine grid + τ_sound)
100 seeds for better statistics
"""
...
function main()
    ...
    α_vals = [0.10, 0.12]
    τ_vals = [0.0, 2.0, 3.0, 4.0, 4.5, 5.0, 5.5, 6.0, 6.5, 7.0, 8.0, 11.0]
    save_times = collect(0.0:DT_SAVE:T_MAX)
    i20 = findfirst(save_times .>= 20.0)
    i50 = findfirst(save_times .>= 50.0)

    for α in α_vals
        @printf("═══ α = %.2f,  g_c = %.5f,  τ_sound/2 ≈ %.2f ═══\n",
                α, g_c, τ_sound/2)
        @printf("%6s  %7s  %7s  %7s  %7s  %7s  %7s\n",
                "τ_r", "τ_r/τ_s", "R₂@pk", "|ρ̂₂|pk", "|ρ̂₂|>20", "|ρ̂₂|>50", "|ρ̂₁|pk")

        for τ_r in τ_vals
            fc_sum = zeros(N_MODES, length(save_times))
            fs_sum = zeros(N_MODES, length(save_times))
            fc_sq  = zeros(N_MODES, length(save_times))
            fs_sq  = zeros(N_MODES, length(save_times))
            ...
            t_wall = @elapsed for seed in 1:N_SEEDS
                ...
                fc_sum .+= fc; fs_sum .+= fs
                fc_sq  .+= fc.^2; fs_sq .+= fs.^2
            end

            fc_m = fc_sum ./ N_SEEDS; fs_m = fs_sum ./ N_SEEDS
            rho_n = sqrt.(fc_m.^2 .+ fs_m.^2)

            fc_var = fc_sq ./ N_SEEDS .- fc_m.^2
            fs_var = fs_sq ./ N_SEEDS .- fs_m.^2
            rho2_std = sqrt(max(0.0, mean(fc_var[2,:]) + mean(fs_var[2,:]))) / sqrt(N_SEEDS)
            ...
            @printf("%6.1f  %7.3f  %7.4f  %7.4f  %7.4f  %7.4f  %7.4f  (%.0fs, σ₂≈%.4f)\n",
                    τ_r, τ_r/τ_sound, R2_pk, rho2_pk, rho2_max20, rho2_max50, rho1_pk,
                    t_wall, rho2_std)
        end
        println()
    end
