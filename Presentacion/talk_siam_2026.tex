...
\begin{columns}[T]
  ...
  \begin{block}{Covariant LWR equation}
    \vspace{2pt}
    In angular coordinates $\vp$:\quad
    $\partial_t(\rho\,h) + \partial_\vp(\rho\,v) = 0$

    \smallskip
    In arc-length $s$:\quad
    $\partial_t \rho + \partial_s[\rho\,v(\rho;\kappa,\tau)] = 0$
    \vspace{2pt}
  \end{block}

  \smallskip
  The flux depends on geometry via a
  \textbf{fundamental diagram}.  We \emph{model}:
  \[
...
\begin{columns}[T]
  ...
  \begin{itemize}
    \setlength\itemsep{5pt}
    \item For constant $\kappa,\tau$ the Riemann problem
          has an exact solution; the scheme
          recovers it to $< 10^{-15}$
    \item Curvature alone: $\mathbf{2.0\%}$ slowdown
    \item Adding torsion: additional $\mathbf{0.07\%}$
    \item These validate the scheme; the \alert{interesting}
          case is variable geometry
  \end{itemize}
...
\begin{columns}[T]
  ...
  (mesoscopic level).
  \textbf{Symbols}: event-driven MD,
  500 hard rods on an ellipse.
  \alert{24 initial conditions},
  no free parameters.}

  \vspace{2pt}
  {\footnotesize The Christoffel term $\Gamma u^2$
  is validated microscopically.
  Traffic LWR is the macroscopic
  phenomenological closure of this hierarchy.}
\end{columns}
\end{frame}
...
\begin{columns}[T]
  ...
  \medskip
  In arc-length $s$ the metric is $1$ by construction,
  so the equation reduces to \textbf{flat-space LWR}
  with position-dependent fundamental diagram.
  The Christoffel term $\Gamma u^2$ is absorbed
